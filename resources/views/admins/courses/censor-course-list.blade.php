    ...
    <div class="m-3">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('admin.courses.approval.history') }}" class="btn btn-primary">Lịch sử kiểm duyệt</a>
            </div>
            <div class="card-body p-3">
                ...
                @if (count($courses) <= 0)
                    <h3 class="text-center">Chưa có khoá học nào cần kiểm duyệt</h3>
                @else
                        ...
                        <tbody>
                            @foreach ($courses as $course)
                                <tr>
                                    ...
                                    <td class="border-end">
                                        @if ($course->status == 'pending')
                                            <a href="{{ route('admin.check.course', $course->id) }}"
                                                class="btn btn-info btn-sm">Kiểm tra</a>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            console.log('Document ready'); // Kiểm tra có chạy tới đây không
            const table = new DataTable('#table');
            if (table) {
                console.log('DataTable plugin loaded'); // Kiểm tra plugin có load không
                // $('#table').DataTable();
            } else {
