...
<body>
    <div class="email-container">
        <div class="email-header">
            <h1>Thông báo kết quả yêu cầu rút tiền</h1>
        </div>
        <div class="email-body">
            <h2>Kính gửi {{ $user->name }}</h2>
            <p>Chúng tôi xin thông báo rằng yêu cầu rút tiền của quý khách đã được chúng tôi xét duyệt với kết quả là:</p>
            <p><strong>
                @if ($statusTransaction === 'success')
                    THÀNH CÔNG
                @elseif($statusTransaction === 'fail')
                    THẤT BẠI
                @endif
                </strong></p>
            <p>
                Ngân hàng: {{ $bank_name }}
                <br>Số tài khoản: {{ $bank_number }}
                <br>Tên tài khoản: {{ $bank_nameUser }}
            </p>
